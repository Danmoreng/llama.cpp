...
class AgenticStore {
	...
	getConfig(settings: SettingsConfigType, perChatOverrides?: McpServerOverride[]): AgenticConfig {
		...
		const hasMcpTools = mcpStore.hasEnabledServers(perChatOverrides);
		const hasEditorTools = editorToolDefinitions.length > 0;

		return {
			enabled: (hasMcpTools || hasEditorTools) && DEFAULT_AGENTIC_CONFIG.enabled,
			maxTurns,
			maxToolPreviewLines
		};
	}
	...
	async runAgenticFlow(params: AgenticFlowParams): Promise<AgenticFlowResult> {
		...
		const hasMcpToolsEnabled = mcpStore.hasEnabledServers(perChatOverrides);
		let mcpInitialized = false;
		let mcpTools: ReturnType<typeof mcpStore.getToolDefinitionsForLLM> = [];

		if (hasMcpToolsEnabled) {
			mcpInitialized = await mcpStore.ensureInitialized(perChatOverrides);
			if (!mcpInitialized && editorToolDefinitions.length === 0) {
		...
		const tools = [...mcpTools, ...editorToolDefinitions];
		if (tools.length === 0) {
	...
	private async executeAgenticLoop(params: {
		...
		for (let turn = 0; turn < maxTurns; turn++) {
			...
			for (const toolCall of normalizedCalls) {
				...
				try {
					if (editorToolDefinitions.some((tool) => tool.function.name === mcpCall.function.name)) {
						result = await executeEditorTool({
							id: toolCall.id,
							type: ToolCallType.FUNCTION,
							function: {
								name: mcpCall.function.name,
								arguments:
